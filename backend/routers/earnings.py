...
def _row_to_earnings(row) -> Earnings:
    return Earnings(
        ...
        ebit_est=float(row["ebit_est"]) if row.get("ebit_est") is not None else None,
        net_income_est=float(row["net_income_est"]) if row.get("net_income_est") is not None else None,
        analyst_estimates=row.get("analyst_estimates") or None,
